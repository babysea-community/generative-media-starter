...
[![BabySea Initiatives](https://custom-icon-badges.demolab.com/badge/babysea-our_initiatives-0D9488?style=for-the-badge&logo=babysea&logoColor=white)](https://babysea.ai/stripe-community-builder)
...
Public starter behavior is limited to this repository: Next.js App Router, Supabase Auth/Postgres/Storage, Stripe one-time credit packs, Supabase RPC credit settlement, Upstash-backed production rate limits, BabySea SDK execution for the default model, and optional Sentry runtime monitoring. Features not implemented here or documented in linked starter docs are out of scope.
...
- Not a full observability platform; Sentry is optional and disabled unless a DSN is configured.
...
Set `NEXT_PUBLIC_SENTRY_DSN` and `NEXT_PUBLIC_SENTRY_ENVIRONMENT` to enable runtime error capture. Leave them blank for local development or forks that do not use Sentry; the starter no-ops cleanly without telemetry.
...
- Sentry is optional and disabled when no DSN is configured.
...
- Keep BabySea, Stripe, Supabase service-role, Upstash, Sentry auth, Vercel, Netlify, and GitHub tokens in deployment secrets only.
- Rotate any secret that was pasted into a terminal, chat, issue, or screenshot.
- Browser code only receives publishable keys.
- Sentry runtime monitoring is optional and disabled when `NEXT_PUBLIC_SENTRY_DSN` is blank; source-map upload uses build/CI-only secrets.
...
- [BabySea SDK](https://github.com/babysea-community/babysea): Production TypeScript SDK used for schema loading, cost estimates, and generation.
